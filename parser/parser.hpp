	...
	char const* error_message)
{
	...
	if ( std::all_of( saved_stream.begin(), saved_stream.end(), isspace ) ) 
	{
		return t;	// empty tree
	}
	...
	IntType value = getInteger<IntType>(
		istr, left_delim, integer_buffer, buffer_size, error_message);
	t.set_root(new typename tree_type::node_type(value));

	tree_type left_tree = getBinaryTree<IntType>(istr, left_delim, right_delim,
		integer_buffer, buffer_size, error_message);
	if (!left_tree.is_empty())
		t.root() -> set_left(left_tree.root());

	tree_type right_tree = getBinaryTree<IntType>(istr, left_delim, right_delim,
		integer_buffer, buffer_size, error_message);
